...
// Funciones de movimiento omnidireccional - CONFIGURACIÓN REAL
void moveForward() {
  // Motor A (izquierda) y B (derecha) adelante, C (atrás) empuja
  setMotor(MOTOR_A_IN3, MOTOR_A_IN4, motorSpeed, true);   // Izquierda adelante
  setMotor(MOTOR_B_IN1, MOTOR_B_IN2, motorSpeed, true);   // Derecha adelante  
  setMotor(MOTOR_C_IN3, MOTOR_C_IN4, motorSpeed/2, true); // Atrás ayuda
}

void moveBackward() {
  // Todos los motores hacia atrás
  setMotor(MOTOR_A_IN3, MOTOR_A_IN4, motorSpeed, false);  // Izquierda atrás
  setMotor(MOTOR_B_IN1, MOTOR_B_IN2, motorSpeed, false);  // Derecha atrás
  setMotor(MOTOR_C_IN3, MOTOR_C_IN4, motorSpeed, false);  // Atrás atrás
}

void moveLeft() {
  // Giro a la izquierda - Motor A lento, B rápido, C ayuda
  setMotor(MOTOR_A_IN3, MOTOR_A_IN4, motorSpeed/3, true);   // Izquierda lento
  setMotor(MOTOR_B_IN1, MOTOR_B_IN2, motorSpeed, true);     // Derecha rápido
  setMotor(MOTOR_C_IN3, MOTOR_C_IN4, motorSpeed/2, false);  // Atrás empuja izquierda
}

void moveRight() {
  // Giro a la derecha - Motor A rápido, B lento, C ayuda
  setMotor(MOTOR_A_IN3, MOTOR_A_IN4, motorSpeed, true);     // Izquierda rápido
  setMotor(MOTOR_B_IN1, MOTOR_B_IN2, motorSpeed/3, true);   // Derecha lento  
  setMotor(MOTOR_C_IN3, MOTOR_C_IN4, motorSpeed/2, true);   // Atrás empuja derecha
}

void rotateLeft() {
  // Rotación pura hacia la izquierda
  setMotor(MOTOR_A_IN3, MOTOR_A_IN4, motorSpeed, false);  // Izquierda atrás
  setMotor(MOTOR_B_IN1, MOTOR_B_IN2, motorSpeed, true);   // Derecha adelante
  setMotor(MOTOR_C_IN3, MOTOR_C_IN4, motorSpeed/2, false); // Atrás ayuda rotación
}

void rotateRight() {
  // Rotación pura hacia la derecha  
  setMotor(MOTOR_A_IN3, MOTOR_A_IN4, motorSpeed, true);   // Izquierda adelante
  setMotor(MOTOR_B_IN1, MOTOR_B_IN2, motorSpeed, false);  // Derecha atrás
  setMotor(MOTOR_C_IN3, MOTOR_C_IN4, motorSpeed/2, true); // Atrás ayuda rotación
}
